...
RUN arduino-cli config init && \
    ...
    if [[ -n "${CUSTOM_IDF}" && -n "${CUSTOM_IDF_DIR}" ]]; then \
        mkdir -p /root/.arduino15/packages/esp32/hardware/esp32 && \
        git clone "${CUSTOM_IDF}" \
          "/root/.arduino15/packages/esp32/hardware/esp32/${CUSTOM_IDF_DIR}" && \
        cd "/root/.arduino15/packages/esp32/hardware/esp32/${CUSTOM_IDF_DIR}" && \
